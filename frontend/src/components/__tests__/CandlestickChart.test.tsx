...
describe("CandlestickChart – rendering", () => {
  it("renders the symbol in the header", () => {
    render(
      <CandlestickChart symbol="AAPL" candles={filledCandles} onClose={vi.fn()} />
    );
  ...
  it("renders interval buttons 1m and 5m", () => {
    render(
      <CandlestickChart symbol="MSFT" candles={filledCandles} onClose={vi.fn()} />
    );
  ...
  it("calls onClose when close button is clicked", () => {
    const onClose = vi.fn();
    render(
      <CandlestickChart symbol="AAPL" candles={filledCandles} onClose={onClose} />
    );
...
describe("CandlestickChart – empty state", () => {
  it("shows collecting message when fewer than 2 candles", () => {
    render(
      <CandlestickChart symbol="AAPL" candles={emptyCandles} onClose={vi.fn()} />
    );
...
describe("CandlestickChart – interval switching", () => {
  ...
  it("shows collecting message after switching if 5m has no candles", () => {
    ...
    render(
      <CandlestickChart symbol="AAPL" candles={partialCandles} onClose={vi.fn()} />
    );
    fireEvent.click(screen.getByRole("button", { name: "5m" }));
    expect(screen.getByText(/Collecting 5m candles/i)).toBeInTheDocument();
  });
