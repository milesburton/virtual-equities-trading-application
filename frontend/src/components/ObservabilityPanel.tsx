...
import { useObservability } from "../hooks/useObservability.ts";
...
function bucketEvents(events: ObsEvent[], bucketMs = 5000) {
  ...
  for (const e of events) {
    ...
    if (String(e.type).includes("no_fill") || String(e.type).includes("reject"))
      buckets[key].rejects += 1;
  }
...
export function ObservabilityPanel() {
  const { events, replay } = useObservability();
          ...
          <ul className="space-y-2">
            {latest.map((e, i) => (
              ...
              >
                ...
                <pre className="text-[11px] text-gray-400 mt-1 overflow-auto max-h-24">
