...
export function AlgoMonitor({ orders }: Props) {
  const [filter, setFilter] = useState<string>("ALL");
  const active = orders.filter(
    (o) =>
      (o.status === "queued" || o.status === "executing") &&
      (filter === "ALL" ? true : o.strategy === filter)
  );
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {active.map((order) => {
                ...
                const msLeft = order.expiresAt - Date.now();
                const secsLeft = Math.max(0, Math.round(msLeft / 1_000));
